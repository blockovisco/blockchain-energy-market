...
def test_connection_request_retry_successful():
    ...
    responses.add(
        responses.GET, "http://127.0.0.1:4003/spongebob", json={"success": True}, status=200
    )
...
def test_connection_raises_for_request_retry_failure():
    ...
    with pytest.raises(SolarHTTPException):
...
def test_handle_response_raises_for_no_content_in_response():
    ...
    with pytest.raises(SolarHTTPException) as exception:
...
@pytest.mark.parametrize("method,func_name", [(responses.GET, "get"), (responses.POST, "post")])
def test_http_methods_call_correct_url_and_return_correct_response(method, func_name):
    responses.add(method, "http://127.0.0.1:4003/spongebob", json={"success": True}, status=200)
